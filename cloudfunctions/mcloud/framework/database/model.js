...
class Model {
	...
	static clearDirtyData(data, dbStructure) {
		// 使用传递的dbStructure或默认的this.DB_STRUCTURE
		const structure = dbStructure || this.DB_STRUCTURE;
		for (let key in data) {
			if (!structure.hasOwnProperty(key) && !key.includes('.')) {
