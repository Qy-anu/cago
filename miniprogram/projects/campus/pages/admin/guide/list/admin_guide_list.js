...
Page({
	...
	onLoad: async function (options) {
		...
		wx.setNavigationBarTitle({
			title: projectSetting.GUIDE_NAME + '-管理',
		});
		this.setData({
			GUIDE_NAME: projectSetting.GUIDE_NAME
		});
	...
	_setSort: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/guide_sort', params).then(res => {
				pageHelper.modifyListNode(id, this.data.dataList.list, 'GUIDE_ORDER', sort);
				this.setData({
	...
	_setVouch: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/guide_vouch', params).then(res => {
				pageHelper.modifyListNode(id, this.data.dataList.list, 'GUIDE_VOUCH', vouch);
				this.setData({
					dataList: this.data.dataList
				});
				pageHelper.showSuccToast('设置成功');
	...
	_del: async function (e) {
		...
		let callback = async () => {
			try {
				...
				await cloudHelper.callCloudSumbit('admin/guide_del', params, opts).then(res => {
	...
	bindMoreTap: async function (e) {
		...
		let order = this.data.dataList.list[idx].GUIDE_ORDER;
			...
			success: async res => {
				switch (res.tapIndex) {
					case 0: {
						let id = pageHelper.dataset(e, 'id');
						wx.navigateTo({
							url: '../../../guide/detail/guide_detail?id=' + id,
						});
						break;
					}
					case 1: {
	...
	_setStatus: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/guide_status', params).then(res => {
				pageHelper.modifyListNode(id, this.data.dataList.list, 'GUIDE_STATUS', status, '_id');
				this.setData({
					dataList: this.data.dataList
				});
				pageHelper.showSuccToast('设置成功');
	...
	_getSearchMenu: function () {
		let cateIdOptions = GuideBiz.getCateList();

		let sortItem1 = [{ label: '分类', type: '', value: 0 }];
		sortItem1 = sortItem1.concat(GuideBiz.getCateList());
