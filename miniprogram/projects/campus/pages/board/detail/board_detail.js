...
Page({
	...
	_loadDetail: async function () {
		...
		let board = await cloudHelper.callCloudData('board/view', params, opt);
		if (!board) {
			this.setData({
				isLoad: null
			})
	...
	_loadComments: async function () {
		...
		let result = await cloudHelper.callCloud('comment/list', {
			oid: id,
			page: 1,
			size: 50,
			isTotal: true,
		}, { title: 'bar' }).catch(() => null);
	...
	bindCommentSubmit: async function () {
		...
		let forms = [];
		if (content) {
			forms.push({ mark: 'content', title: '评论内容', type: 'textarea', val: content });
		}
		if (images.length) {
			forms.push({ mark: 'img', title: '图片', type: 'image', val: images });
		}
	...
	onShareAppMessage: function (res) {
		return {
			title: this.data.board.BOARD_OBJ.to || '心情详情',
			imageUrl: this.data.board.BOARD_OBJ.pic && this.data.board.BOARD_OBJ.pic[0]
		} 
	}
})