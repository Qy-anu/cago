			...
			success: function (res) {
				if (res.result.code == CODE.LOGIC || res.result.code == CODE.DATA) {
					...
					if (hint) {
						wx.showModal({
							title: '温馨提示',
							content: (res.result && res.result.msg) ? res.result.msg : '系统开小差了，请稍后重试',
							showCancel: false
						});
					}
					reject(res.result);
...
async function normalizeCloudFileId(fileID) {
...
async function getTempFileURLOne(fileID) {
	fileID = await normalizeCloudFileId(fileID);
	if (!fileID) return '';

	let result = await wx.cloud.getTempFileURL({
		fileList: [fileID],
	})
	console.log('[getTempFileURLOne] fileID=', fileID, 'result=', result);
